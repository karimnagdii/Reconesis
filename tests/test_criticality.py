...
def test_bundle_scripts_empty_when_no_scripts():
    host = _make_host(ports=[{"port": 80, "service": "http"}])
    assessment = assessor.assess(host)
    bundle = assessor.build_evidence_bundle(host, assessment)
    assert bundle["scripts"] == {}

def test_bundle_generic_host_top_score_zero():
...
def test_bundle_runner_up_is_second_highest():
    """Runner-up must be the second-highest scoring named profile."""
    host = _make_host(ports=[{"port": 5432, "service": "postgresql", "product": "postgresql"}])
    assessment = assessor.assess(host)
    bundle = assessor.build_evidence_bundle(host, assessment)
    ...
    assert bundle["static"]["runner_up"]["type"] != "Generic Host" or bundle["static"]["runner_up"]["score"] == 0

def test_bundle_ports_include_service_product_version():
...
def test_bundle_signals_from_assess_reasons():
